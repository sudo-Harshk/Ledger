<script lang="ts">
  ...
  import { ChevronLeft, ChevronRight, TrendingUp, TrendingDown, Minus } from '@lucide/svelte';
  ...
  let prevSummary = $state({ income: 0, expense: 0, net: 0, transactions: [] as any[] });

  $effect(() => { loadData(month); });

  async function loadData(m: string) {
    [summary, catSpend, dailyData, prevSummary] = await Promise.all([
      getMonthSummary(m),
      getCategorySpend(m),
      getDailySpend(m),
      getMonthSummary(prevMonth(m))
    ]);
  }

  const expenseDiff    = $derived(summary.expense - prevSummary.expense);
  const expenseDiffPct = $derived(
    prevSummary.expense > 0 ? ((expenseDiff / prevSummary.expense) * 100).toFixed(0) : null
  );
</script>
...
<div class="px-4 pt-6 animate-fade-in">
  ...
  {#if expenseDiffPct !== null}
    <div class="bg-[var(--color-surface)] rounded-2xl p-4 mb-5 flex items-center gap-3">
      {#if expenseDiff > 0}
        <TrendingUp size={20} class="text-[var(--color-expense)] shrink-0" />
        <p class="text-sm">
          <span class="font-semibold text-[var(--color-expense)]">+{expenseDiffPct}%</span>
          <span class="text-[var(--color-text-muted)]"> more than last month</span>
        </p>
      {:else if expenseDiff < 0}
        <TrendingDown size={20} class="text-[var(--color-income)] shrink-0" />
        <p class="text-sm">
          <span class="font-semibold text-[var(--color-income)]">{expenseDiffPct}%</span>
          <span class="text-[var(--color-text-muted)]"> less than last month</span>
        </p>
      {:else}
        <Minus size={20} class="text-[var(--color-text-muted)] shrink-0" />
        <p class="text-sm text-[var(--color-text-muted)]">Same as last month</p>
      {/if}
    </div>
  {/if}
